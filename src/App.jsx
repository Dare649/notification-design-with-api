...
const App = () => {
  ...
  return (
    <div className='container mt-20 mx-auto px-6 sm:w-2/4' >
      ...
      {
        post.map((d, i) =>(
          <ul role='list' key={i.id} className='py-6 divide-y divide-slate-200 flex hover:bg-slate-200 hover:rounded'>
            <div className='flex'>
              <img src="./src/images/blue-dot-icon.png" className='w-4 h-4 mt-1'/>
              <div className='px-2'>
                <li className='text-lg hover:text-blue-500'>{d.message}</li>
                <li>{d.datetime}</li>
            </div>
            </div>
